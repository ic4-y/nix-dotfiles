...
let
  ...
  testLib = {
          ...
          lib.foldlAttrs
            (acc: n: v: {
              ...
              grub-devices =
                acc.grub-devices
                ++ (lib.optional
                  (lib.any (part: (part.type or "") == "EF02") (
    ...
    makeDiskoTest =
      ...
      , postDisko ? ""
      , testMode ? "module"
      , # can be one of direct module cli
        testBoot ? true
      , # if we actually want to test booting or just create/mount
        enableOCR ? false
      ,
      }:
      let
        ...
        diskoConfigWithArgs =
          if builtins.isFunction importedDiskoConfig then
            importedDiskoConfig { inherit pkgs lib; }
          else
            importedDiskoConfig;
